...
import { Header, Footer, TakeNote, ViewNotesList, SideBar } from "./components/index.js";

function NotesHub() {
  const [pinnedNotesList, setPinnedNotesList] = useState([]);
  const [otherNotesList, setOtherNotesList] = useState([]);
  const [tagOptions,setTagOptions] = useState([{value:'urgent',name:'Urgent'},{value:'imp',name:'IMP'},{value:'read',name:'Read'}]);
